...
public sealed class PrintersController(IMediator mediator) : ControllerBase
{
    ...
    public async Task<IActionResult> Add(AddPrinterRequest body, CancellationToken ct)
    {
        var dto = await mediator.Send(
            new AddPrinterCommand(body.Name, body.IpAddress, body.Location ?? "", body.Community ?? "public", body.PaperTrayCount ?? 0), ct);
    ...
    public async Task<IActionResult> Update(
        ...
        Ok(await mediator.Send(new UpdatePrinterCommand(ip, body.Name, body.Location, body.Community, body.PaperTrayCount), ct));
...
// Request body models
public sealed record AddPrinterRequest(string Name, string IpAddress, string? Location, string? Community, int? PaperTrayCount);
public sealed record UpdatePrinterRequest(string? Name, string? Location, string? Community, int? PaperTrayCount);
